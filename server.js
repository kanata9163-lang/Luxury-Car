...
// ── 画像プロキシ（GooNet 等のホットリンク防止を回避）─────────────────────────
app.get('/api/img', async (req, res) => {
  let imgUrl = req.query.url;
  if (!imgUrl) return res.status(400).send('Missing url parameter');

  // Auto-upgrade GooNet thumbnail → large
  if (imgUrl.includes('/J/')) imgUrl = imgUrl.replace('/J/', '/L/');
  ...
  try {
    const response = await ax.get(imgUrl, { responseType: 'arraybuffer', timeout: 15000, headers, validateStatus: s => s === 200 });
    const ct = response.headers['content-type'] || 'image/jpeg';
    res.set({ 'Content-Type': ct, 'Cache-Control': 'public, max-age=86400' });
    res.send(Buffer.from(response.data));
  } catch (err) {
    // Fallback to /J/ if /L/ not available
    if (imgUrl.includes('/L/')) {
      try {
        const resp2 = await ax.get(imgUrl.replace('/L/', '/J/'), { responseType: 'arraybuffer', timeout: 15000, headers });
